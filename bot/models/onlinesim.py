import sqlalchemy as sa
from bot.models import BaseModel
import datetime
import pytz
import enum


class OnlinesimStatus(enum.Enum):
    waiting = 0
    success = 1
    cancel = 2
    expire = 3
    error = 4


class Onlinesim(BaseModel):
    __tablename__ = 'onlinesim'

    id = sa.Column(sa.Integer, unique=True, primary_key=True, index=True, autoincrement=True)
    user_id = sa.Column(sa.Integer, sa.ForeignKey('users.user_id'))
    tzid = sa.Column(sa.Integer, nullable=False)
    telephone_number = sa.Column(sa.Integer)
    service_code = sa.Column(sa.Text, nullable=False)
    country_code = sa.Column(sa.Text, nullable=False)
    status = sa.Column(sa.Enum(OnlinesimStatus), default=OnlinesimStatus.waiting)
    msg = sa.Column(sa.Text, default=None)
    date = sa.Column(sa.DateTime, default=datetime.datetime.now(pytz.timezone('Europe/Moscow')))

    user = sa.orm.relationship("User", backref="payments")
